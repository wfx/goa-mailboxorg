...
#include "goamborgprovider.h"
#include "goaobjectskeletonutils.h"
#include "goautils.h"

struct _GoaMailboxOrgProvider
{
  GoaProvider parent_instance;
};

G_DEFINE_TYPE_WITH_CODE (GoaMailboxOrgProvider, goa_mailboxorg_provider, GOA_TYPE_PROVIDER,
...
static const gchar *CALDAV_ENDPOINT = "caldav/";
static const gchar *CARDDAV_ENDPOINT = "carddav/";
static const gchar *WEBDAV_ENDPOINT = "servlet/webdav.infostore";

static const gchar *
get_provider_type (GoaProvider *provider)
{
  return GOA_MAILBOXORG_NAME;
}

static gchar *
get_provider_name (GoaProvider *provider, GoaObject *object)
{
  return g_strdup(_("MailboxOrg"));
}
              ...
              GError             **error)
{
  ...
  /* Chain up */
  if (!GOA_PROVIDER_CLASS (goa_mailboxorg_provider_parent_class)->build_object (provider,
                                                                              object,
                                                                              key_file,
                                                                              group,
                                                                              connection,
  ...
  if (g_cancellable_is_cancelled (data.cancellable))
  ...
  else if (data.error != NULL)
    {
      ...
      markup = g_strdup_printf ("<b>%s:</b>\n%s",
                                _("Error connecting to MailboxOrg server"),
                                data.error->message);
                 ...
                 GError        **error)
{
  ...
  g_return_val_if_fail (GOA_IS_MAILBOXORG_PROVIDER (provider), FALSE);
...
static void
goa_mailboxorg_provider_init (GoaMailboxOrgProvider *self)
{
}

static void
goa_mailboxorg_provider_class_init (GoaMailboxOrgProviderClass *klass)
{
